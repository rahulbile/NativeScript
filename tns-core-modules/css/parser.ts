...
type InputToken = "(" | ")" | "{" | "}" | "[" | "]" | ":" | ";" | "," | " " | "^=" | "|=" | "$=" | "*=" | "~=" | "<!--" | "-->" | undefined /* <EOF-token> */ | InputTokenObject;
...
export class CSSParser {
    ...
    private consumeAStringToken(): InputTokenObject {
        ...
        return { type: TokenObjectType.string, text: result[1] };
    }
    ...
    public consumeAListOfRules(): Rule[] {
        ...
        while(inputToken = this.consumeAToken()) {
            ...
            if (qualifiedRule) rules.push(qualifiedRule);
        }
        return rules;
    }
    ...
    public consumeAnAtRule(): AtRule {
        ...
        while(inputToken = this.consumeAToken()) {
            ...
            if (component) atRule.prelude.push(component);
        }
        return atRule;
    }
    ...
    private consumeAComponentValue(): InputToken {
        ...
        } else if (inputToken.type === TokenObjectType.function) {
            return this.consumeAFunction(inputToken.text);
        }
        return inputToken;
    }
    ...
    private consumeASimpleBlock(associatedToken: InputToken): SimpleBlock {
        ...
        while(inputToken = this.consumeAToken()) {
            ...
            if (value) block.values.push(value);
        }
        return block;
    }
    ...
    private consumeAFunction(name: string): InputToken {
        const funcToken: FunctionInputToken = { type: TokenObjectType.function, text: undefined, components: [] };
        do {
            if (this.nextInputCodePointIndex >= this.text.length) {
                return funcToken;
            }
