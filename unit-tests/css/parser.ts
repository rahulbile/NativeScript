...
describe("css", () => {
    describe("parser", () => {
        ...
        describe("stylesheet", () => {
            it.only("parse theme", () => {
                console.time("rework-css");
                const rework = parse(themeCoreLightIos, { source: "nativescript-theme-core/css/core.light.css" });
                console.timeEnd("rework-css");

                console.time("shady-css-parser");
                const parser = new shadyCss.Parser();
                const ast = parser.parse(themeCoreLightIos);
                console.timeEnd("shady-css-parser");

                console.time("nativescript");
                const cssparser = new StylesheetParser(themeCoreLightIos);
                const stylesheet = cssparser.parseAStylesheet();
                console.timeEnd("nativescript");
            });
        });
    });
});
